...
import java.util.*;
public class ChampPanel extends CustomPanel 
{
    private RiotStore store;
    public ChampPanel(RiotStore store)
    {  
        addTitle("Buy a Champion");
        setup();
        this.store = store;
        store.attach(this);
        build();
    ...
    public void build()
    {   
        add(new TheChampionPanel(store)); 
    }
    public void close()
    {
        setVisible(false);
    }
    private class TheChampionPanel extends CustomPanel implements MyObserver
    {
        private RiotStore store;
        ...
        public TheChampionPanel(RiotStore store)
        {
            this.store = store;
            store.attach(this);
            build();
        ...
        private void build()
        {
            addPair("Champion Name", Title);
            addPair("Champion Cost", cost);
